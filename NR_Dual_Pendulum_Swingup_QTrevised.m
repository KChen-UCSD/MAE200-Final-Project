function [u_k,x_k,t]=NR_Dual_Pendulum_Swingup_QTrevised(T,u_k)
...
s.m1=1; s.L1=1;    s.ell1=s.L1/2; s.I1=s.m1*s.ell1^2/12;
s.m2=0.5; s.L2=0.5;  s.ell2=s.L2/2; s.I2=s.m2*s.ell2^2/12; alpha=0.1;
s.B=[0; 0; 0; 1; 0; 0]; s.Q=diag([0 0 0 0 0 0]); s.R=0; s.QT=diag([50, 40, 10, 0.1, 6000, 10000]);
...
for n=1:s.N+1      % Compute u_k corresponding to different s.mc to give same x_k
  E=Compute_E(x_k(1:6,n),s); N=Compute_N(x_k(1:6,n),0,s); u_k(n,1)=s.B'*(E*x_k(4:9,n)-N); % A=Compute_A(x_k(:,n),s);
end
